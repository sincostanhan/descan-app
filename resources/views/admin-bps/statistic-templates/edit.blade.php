            ...
            <x-section-card title="Sumber Baris" title-size="text-xl" class="mb-6">
                    <div class="flex flex-col gap-2">
                        ...
                        <label class="label cursor-pointer justify-start gap-3">
                            ...
                            {{-- Panel ini sengaja bersarang di dalam opsi radio rt_rw, disembunyikan lewat toggleRowSourceMode()
                                kalau BPS pilih mode manual — total per RW cuma masuk akal kalau barisnya memang per-RT. --}}
                            <div id="rt-rw-totals-panel" class="ml-9 mt-2 flex flex-col gap-2 {{ old('row_source', $statistic_template->row_source ?? 'manual') !== 'rt_rw' ? 'hidden' : '' }}">
                                <label class="label cursor-pointer justify-start gap-3 w-fit">
                                    <input type="checkbox" name="show_rw_subtotal" value="1" class="checkbox checkbox-sm"
                                        {{ old('show_rw_subtotal', $statistic_template->show_rw_subtotal ?? false) ? 'checked' : '' }}>
                                    <span class="label-text">Tampilkan baris "Total per RW" (dihitung otomatis)</span>
                                </label>
                                <label class="label cursor-pointer justify-start gap-3 w-fit">
                                    <input type="checkbox" name="show_kelurahan_total" value="1" class="checkbox checkbox-sm"
                                        {{ old('show_kelurahan_total', $statistic_template->show_kelurahan_total ?? false) ? 'checked' : '' }}>
