<x-layout-admin title="Pratinjau Tabel Statistik">
                            ...
                            bg-base-50 p-6">
                                ...
                                <div class="space-y-4">
                                    <div class="alert alert-info shadow-sm mb-4">
                                        ...
                                        <span><strong>Opsional</strong><br>Anda dapat langsung mengatur visualisasi grafik di sini.</span>
                                    </div>

                                    <fieldset class="fieldset w-full">
                                        <legend class="fieldset-legend text-base">Tipe Grafik</legend>
                                        <select name="chart_type" class="select w-full">
                                            <option value="">-- Pilih Tipe Grafik (Abaikan jika tidak ingin membuat) --</option>
                                            ...
                                            <option value="bar_clustered" {{ old('chart_type') == 'bar_clustered' ? 'selected' : '' }}>Bar Chart (Clustered)</option>
                                            <option value="bar_stacked" {{ old('chart_type') == 'bar_stacked' ? 'selected' : '' }}>Bar Chart (Stacked)</option>
                                            <option value="column_clustered" {{ old('chart_type') == 'column_clustered' ? 'selected' : '' }}>Column Chart (Clustered)</option>
                                            <option value="column_stacked" {{ old('chart_type') == 'column_stacked' ? 'selected' : '' }}>Column Chart (Stacked)</option>
                                            <option value="line_markers" {{ old('chart_type') == 'line_markers' ? 'selected' : '' }}>Line Chart with Markers</option>
                                            {{-- <option value="line_markers" {{ old('chart_type') == 'line_markers' ? 'selected' : '' }}>Line Chart</option> --}}
                                        </select>
                                    ...
                                    <fieldset class="fieldset w-full">
                                        ...
                                        <input type="text" name="chart_title" value="{{ old('chart_title') }}" placeholder="Default: Mengikuti Judul Tabel" class="input w-full"/>
                                        <x-forms.error name="chart_title" />
                                    </fieldset>

                                    <fieldset class="fieldset w-full">
                                        <legend class="fieldset-legend text-base">Sumbu X (Kategori Utama)</legend>
                                        {{-- <select name="x_axis_column" class="select w-full"> --}}
                                        <select name="x_axis_column" id="xAxisSelect" class="select w-full">
                                            <option value="">-- Pilih Kolom Sumbu X --</option>
                                            @foreach($columns as $col)
                                                <option value="{{ $col }}" {{ old('x_axis_column') == $col ? 'selected' : '' }}>{{ $col }}</option>
                                            @endforeach
                                        </select>
                                    ...
                                    <fieldset class="fieldset w-full mt-4">
                                        ...
                                        >
                                            @php
                                                // Array warna default (palet elegan)
                                                ...
                                                <div class="flex items-center gap-2 p-1 y-axis-wrapper">
                                                    <label class="cursor-pointer label justify-start gap-2 flex-grow">
                                                        <input type="checkbox" name="y_axis_columns[]" value="{{ $col }}" class="checkbox checkbox-primary checkbox-sm y-axis-checkbox" 
                                                            {{ in_array($col, old('y_axis_columns', [])) ? 'checked' : '' }} />
                                                        <span class="label-text truncate">{{ $col }}</span>
                                                    </label>
                                                    <input type="color" name="y_axis_colors[{{ $col }}]" 
                                                        id="color_{{ Str::slug($col) }}"
                                                        value="{{ old('y_axis_colors.'.$col, $defaultColors[$index % count($defaultColors)]) }}" 
                                    ...
                                    {{-- <fieldset class="fieldset mt-2"> --}}
                                        <label class="cursor-pointer label justify-start gap-3">
                                            <input type="checkbox" name="is_chart_active" value="1" class="toggle toggle-primary" checked />
                                ...
                                {{-- <div id="chartPreviewContainer" class="hidden mt-8 mb-4 border rounded-xl p-4 bg-white shadow-sm w-full">
                                    ...
                                    <div style="position: relative; height:400px; width:100%">
                                        <canvas id="liveChartPreview"></canvas>
                                    </div>
                                </div> --}}
                                <div id="chartPreviewContainer" class="hidden mt-8 mb-4 border rounded-xl p-4 bg-white shadow-sm w-full">
                                    <h3 class="text-center font-bold text-gray-700 mb-4">Live Preview Grafik</h3>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const chartsGrid = document.getElementById('chartsGrid'); // Wadah baru
            
            let chartInstances = []; // Sekarang array, karena bisa ada banyak grafik!
            ...
            // Fungsi pembantu untuk membersihkan semua grafik lama
            function destroyAllCharts() {
                chartInstances.forEach(chart => chart.destroy());
                chartInstances = [];
                chartsGrid.innerHTML = ''; // Kosongkan wadah div
            }

            // Fungsi pembantu untuk membuat elemen Canvas baru
            function createCanvasContainer(titleText = null) {
                // Buat wrapper div agar bisa di-grid dan dikasih judul
            ...
            function updatePreview() {
                ...
                // Hapus semua grafik lama sebelum render yang baru
                destroyAllCharts();
                ...
                // FUNGSI BANTUAN: Ubah warna HEX (#3b82f6) jadi format RGB angka (59, 130, 246)
                ...
                if (type === 'pie' || type === 'doughnut') {
                    // JIKA PIE/DOUGHNUT: Buat 1 Grafik untuk SETIAP Sumbu Y yang dicentang
                    
                    // Ubah grid container menjadi fleksibel (1 kolom jika 1 grafik, 2 kolom jika >= 2 grafik)
                    chartsGrid.className = checkedYAxes.length > 1 
                    ...
                    checkedYAxes.forEach((yCol) => {
                        ...
                        // Ambil warna yang dipilih admin dari input color
                        // Kita pakai ID yang digenerate Laravel Str::slug untuk mencari elemennya
                        // Karena di JS agak susah pakai regex, kita cari dari elemen saudaranya saja:
                        ...
                        const baseHexColor = colorInput.value;
                        ...
                        // // LOGIKA GRADASI TRANSPARANSI SESUAI IDE ANDA:
                        // // Kita buat opacity dari 1.0 turun hingga 0.3 (agar yang paling terang masih terlihat)
                        // const pieColors = labels.map((_, i) => {
                        //     const minOpacity = 0.25; // Transparansi paling rendah
                        //     // Rumus: kurangi opacity perlahan berdasarkan jumlah label
                        //     const opacity = labels.length > 1 
                        //         ? 1 - (i * ((1 - minOpacity) / (labels.length - 1))) 
                        //         : 1;
                        ...
                        const newChart = new Chart(ctx, {
                            ...
                            data: {
                                ...
                                datasets: [{
                                    ...
                                    data: sortedData,
                                    backgroundColor: pieColors, // Masukkan Array warna gradasi
                                    borderColor: `#ffffff`, // Garis batas putih agar potongannya rapi
                                    borderWidth: 1
                                }]
                            },
                            options: { 
                    ...
                    // JIKA BAR/LINE/COLUMN: Gambar semuanya di SATU Grafik
                    
                    // Reset grid container jadi 1 kolom besar
                    chartsGrid.className = 'grid grid-cols-1 w-full';
                    const ctx = createCanvasContainer(); // Buat 1 kanvas tanpa judul kecil
                    
                    datasets = checkedYAxes.map((yCol) => {
                        // Ambil warna yang dipilih admin
                        const checkbox = Array.from(yAxisCheckboxes).find(cb => cb.value === yCol);
                        const colorInput = checkbox.closest('.y-axis-wrapper').querySelector('.color-picker-input');
                        ...
                        return {
                            label: yCol,
                            data: dataToRender.map(row => parseFloat(row[yCol]) || 0),
                            backgroundColor: hexColor, // Warna solid
            ...
            // Pasang listener pada color picker
            const colorPickers = document.querySelectorAll('.color-picker-input');
            colorPickers.forEach(picker => {
                picker.addEventListener('input', updatePreview); // Pakai 'input' agar berubah saat warna digeser
            });
